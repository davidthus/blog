<script lang="ts">
	...
	const story = docStore<Story>(`/stories/${data.id}`);
	const chapters = collectionStore<Chapter>(`/stories/${data.id}/chapters`);
</script>

<section
	class="flex flex-col items-center py-8 px-4 flex-1 mx-10 gap-4 bg-background/80 rounded-2xl shadow-lg shadow-black/30"
>
	<h1 class="font-roman text-text text-6xl">{$story?.name}</h1>
	...
	<ul class="flex flex-col w-full gap-2">
		{#each $chapters as chapter, i}
			<li class="w-full rounded-md py-2 px-4 shadow-md shadow-slate-900/70">
				<a href={`/stories/${data.id}/chapters/${chapter.id}`}>
