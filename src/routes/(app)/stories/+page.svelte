...
<section class="flex flex-col flex-1 ml-10 gap-4 max-w-xl">
	...
	{#each $collection as story}
		<a href="/stories/{story.id}">
			<article
				class="w-full p-6 flex flex-col items-start gap-2 rounded-2xl shadow-lg shadow-slate-900 hover:cursor-pointer"
			>
				<h1 class="font-roman text-text text-2xl">{story?.name}</h1>
				<p class="font-sharp text-text text-base">{story?.description}</p>
			</article>
		</a>
	{/each}
</section>
